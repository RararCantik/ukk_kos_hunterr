// lib/view/owner/tambah_kost.dart
import 'dart:io';
...
class TambahKost extends StatefulWidget {
  const TambahKost({super.key});

  @override
  State<TambahKost> createState() => _TambahKostState();
}

class _TambahKostState extends State<TambahKost> {
  ...
  Future<void> _saveKost() async {
    ...
    if (mounted) {
      if (sukses) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Kost berhasil disimpan!')),
        );
        Navigator.pop(context);
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(ctrl.errorMessage ?? 'Gagal menyimpan')),
        );
      }
    }
  }
              ...
              child: ListView(
                ...
                children: [
                  ...
                  ElevatedButton(
                    ...
                    child: const Text('Simpan Kost', style: TextStyle(fontSize: 16)),
                  ),
                ],
              ),
            ),
      ...
      children: [
        ...
        if (_images.isNotEmpty)
          TextButton.icon(
            ...
            label: const Text('Tambah/Ubah Foto'),
          ),
      ],
    );
  }
